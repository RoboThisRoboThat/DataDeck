...
const TableNode = ({
	...
	return (
		<div
			className="bg-white dark:bg-gray-800 rounded-md border border-gray-200 dark:border-gray-700 shadow-md overflow-hidden"
			style={{
				borderLeft: data.groupColor
					? `4px solid ${data.groupColor}`
					: undefined,
			}}
		>
				...
				{data.columns.map((col) => (
					...
					>
						<div className="flex items-center gap-1">
							...
							<span
								className={`${col.isPrimary ? "font-semibold" : ""} ${col.isForeign ? "text-blue-500" : ""}`}
							>
								{col.name}
							</span>
						</div>
...
const ERDiagram = ({ connectionId }: ERDiagramProps) => {
	...
	useEffect(() => {
		const fetchSchema = async () => {
			...
			try {
				...
				const { data: schemaData } = await window.database.getDatabaseSchema(
					connectionId,
					false,
				);
		...
		(schema: TableSchema[]) => {
			...
			const groupDimensions = mergedComponents.map((component) => {
				...
				if (totalTableCount <= 10) {
					...
					return {
						width: Math.max(
							maxTableWidth * gridSize + (gridSize - 1) * 150 + 200,
							1000,
						),
						height: Math.max(
							maxTableHeight * gridSize + (gridSize - 1) * 150 + 200,
							800,
						),
					};
				}
			...
			const groupNames = mergedComponents.map((component) => {
				...
				const shortestTableName = [...component].sort(
			...
			const tableNodes: Node[] = schema.map((table) => {
				...
				const dimensions = tableDimensions.get(table.name) || {
					width: 220,
					height: 150,
				};
				...
				return {
					...
					data: {
						...
						columns: table.columns.map((col) => ({
							...col,
							isForeign:
								foreignKeyMap.get(table.name)?.includes(col.name) || false,
						})),
					},
					style: {
			...
			for (const table of schema) {
				for (const fk of table.foreignKeys) {
					try {
						...
						const referencedTable = schema.find(
							(t) => t.name === fk.referencedTable,
						);

						if (!referencedTable) {
							console.warn(
								`Referenced table ${fk.referencedTable} not found for foreign key ${table.name}.${fk.column}`,
							);
							continue;
						}
						...
						console.error(
							`Error creating edge for ${table.name}.${fk.column}:`,
							err,
						);
					}
				}
		...
		for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
			...
			for (const groupIndex of rows[rowIndex]) {
				maxHeightInRow = Math.max(
					maxHeightInRow,
					groupDimensions[groupIndex].height,
				);
			}
	...
	const layoutIsolatedTables = (
		...
		const columns = Math.max(
			1,
			Math.floor((groupWidth - padding) / (avgTableWidth + padding)),
		);

		// Distribute tables in a grid
		tables.forEach((table, index) => {
			const dimensions = tableDimensions.get(table) || {
				width: 220,
				height: 150,
			};
	...
	const layoutTablesInGroup = (
		...
		const gridColumns = Math.ceil(
			Math.sqrt(tables.length * (avgTableWidth / avgTableHeight)),
		);
		...
		tables.forEach((table, index) => {
			...
			const y =
				padding + row * cellHeight + (cellHeight - dimensions.height) / 2;
			...
			<div className="h-full flex items-center justify-center">
				<div className="max-w-md p-6 bg-background border rounded-md">
					...
					<p className="text-muted-foreground mb-4">
						This database doesn't appear to have any tables or they couldn't be
						loaded.
					</p>
					<Button onClick={handleRefresh}>Refresh Schema</Button>
				</div>
			</div>
		);
	}
