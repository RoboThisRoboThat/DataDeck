...
export function ConnectionScreen() {
    ...
    if (!isLoading) {
      try {
        ...
        console.log('Settings loaded:', {
          openai: openaiKeyValue ? `*****${openaiKeyValue.slice(-4)}` : '(empty)',
          claude: claudeKeyValue ? `*****${claudeKeyValue.slice(-4)}` : '(empty)'
        });
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      console.log('OpenAI Key:', openaiKey ? `*****${openaiKey.substring(openaiKey.length - 4)}` : 'Not set');
      console.log('Claude Key:', claudeKey ? `*****${claudeKey.substring(claudeKey.length - 4)}` : 'Not set');
          ...
          <div className="grid gap-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Enter your API keys to enable AI features in Data Deck. Your keys are stored securely and encrypted.
              </p>
              
              <div className="grid gap-4 sm:gap-6">
                <div className="grid gap-2">
                  <Label htmlFor="openai-key" className="text-left flex items-center gap-1">
                    OpenAI API Key
                  ...
                  <p className="text-sm text-muted-foreground">
                    Used for GPT models like GPT-4. <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Get your API key</a>
                  </p>
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="claude-key" className="text-left flex items-center gap-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Used for Claude models. <a href="https://console.anthropic.com/settings/keys" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Get your API key</a>
                  </p>
                </div>
              </div>
            </div>
    ...
    <Layout title="Data Deck" showThemeToggle={false}>
      <div className="flex-1 p-4 overflow-auto">
        <div className="max-w-4xl mx-auto">
          ...
          {activeTab === 'connections' ? renderConnectionsTab() : renderSettingsTab()}
        </div>
      </div>
